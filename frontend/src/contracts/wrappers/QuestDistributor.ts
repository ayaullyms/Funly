...
export function getContractCode(): Cell {
    if (COMPILED_HEX === 'REPLACE_WITH_COMPILED_HEX_AFTER_tact_build') {
        throw new Error(
            'Скомпилируй контракт:\n' +
            '  cd quest-contracts\n' +
            '  npx blueprint build QuestDistributor\n' +
            'Затем вставь hex из build/QuestDistributor.compiled.json'
        );
    }
