...
const C = {
  ...
  purple: '#7B6EF6', purpleL: '#9d90f8', muted: '#44445a',
  sec: '#888', green: '#4ade80', red: '#f87171',
};
    ...
    <div className="flex flex-col gap-4 pb-4">
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <button onClick={onBack} style={backBtnSt}>← Tasks</button>
          <span style={{ fontSize: 11, color: C.purpleL, fontFamily: 'IBM Plex Mono, monospace', fontWeight: 700 }}>
            {num} / {total}
          </span>
        </div>

        {/* progress bar */}
        <div style={{ height: 2, background: '#2a2a3a', borderRadius: 2, marginBottom: 10 }}>
          <div style={{ height: '100%', borderRadius: 2, background: C.purple, width: `${progress}%`, transition: 'width 0.3s' }} />
        </div>

        <div style={{ display: 'inline-flex', background: 'rgba(123,110,246,0.15)', border: '0.5px solid rgba(123,110,246,0.35)', borderRadius: 5, padding: '2px 8px', fontSize: 10, color: C.purpleL, fontWeight: 700, marginBottom: 8 }}>
          +{task.points} pts
        </div>
        <h2 style={{ fontWeight: 700, fontSize: 15, color: '#fff', lineHeight: 1.5, marginBottom: 3 }}>{task.title}</h2>
        {task.description && (
          <p style={{ fontSize: 11, color: '#6a609a', lineHeight: 1.5 }}>{task.description}</p>
        )}
      </div>
      ...
      {submitted ? (
        ...
        <p style={{ color: C.sec, textAlign: 'center', fontSize: 13, padding: '1rem 0' }}>Quest is not active</p>
      ) : isMulti ? (
          ...
          {opts.map((o, i) => {
            ...
            return (
              <button key={i} onClick={() => setSelectedIndex(i)} style={{
                ...
                <span style={{
                  width: 18, height: 18, borderRadius: '50%', border: `1px solid ${isSel ? C.purple : '#2a2a3a'}`,
                  background: isSel ? C.purple : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                }}>
                  {isSel && <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#fff', display: 'block' }} />}
                </span>
                <span style={{ fontSize: 12, color: isSel ? '#d0c8ff' : '#bbb' }}>{o}</span>
              </button>
            );
          })}
        ...
        <div className="flex flex-col gap-3">
          <input
            ...
            style={{
              background: C.bg2, border: `0.5px solid rgba(123,110,246,0.5)`, borderRadius: 9,
              padding: '11px 13px', fontSize: 13, color: '#ddd',
              fontFamily: 'IBM Plex Sans, sans-serif', outline: 'none', width: '100%',
            }}
          />
          <button onClick={submit} disabled={!textAns.trim() || submitting} style={primaryBtnSt(!!textAns.trim() && !submitting)}>
            {submitting ? 'Checking...' : 'Check Answer'}
          </button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 5, justifyContent: 'center' }}>
        {tasks.map((t, i) => {
          ...
          return <div key={i} style={{ width: 18, height: 3, borderRadius: 2, background: bg }} />;
        })}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        {currentIndex > 0 && (
          <button onClick={() => setCurrentIndex(i => i - 1)} style={ghostBtnSt}>← Prev</button>
        )}
        {currentIndex < total - 1 ? (
          <button onClick={() => setCurrentIndex(i => i + 1)} style={{ ...ghostBtnSt, flex: 1 }}>Next →</button>
        ) : (
          <button onClick={onBack} style={{ ...primaryBtnSt(true), flex: 1 }}>View Results</button>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-3">
      {opts.length > 0 ? opts.map((o, i) => {
        ...
        <div style={{ background: '#13131f', border: '0.5px solid #2a2a3a', borderRadius: 10, padding: '12px 14px' }}>
          <div style={{ fontSize: 9, color: '#555', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 4, fontFamily: 'IBM Plex Mono, monospace' }}>Your answer</div>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#ccc' }}>{task.myAnswer || '—'}</div>
        </div>
      )}
      {/* Result banner */}
      <div style={{
        ...
        <span style={{ fontSize: 13, fontWeight: 700, color: isCorrect ? '#4ade80' : '#f87171' }}>
          {isCorrect ? 'Correct!' : 'Wrong answer'}
        </span>
        {isCorrect && (
          <span style={{ fontSize: 18, fontWeight: 800, color: '#4ade80', fontFamily: 'IBM Plex Mono, monospace' }}>
            +{task.myPoints}
          </span>
        )}
      </div>
...
const backBtnSt: React.CSSProperties = {
  fontSize: 11, color: '#7B6EF6', background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: 'IBM Plex Sans, sans-serif',
};

const primaryBtnSt = (active: boolean): React.CSSProperties => ({
  width: '100%', background: active ? '#7B6EF6' : '#2a2a3a', color: '#fff', fontSize: 13, fontWeight: 700,
  padding: '10px 14px', borderRadius: 9, border: 'none', cursor: active ? 'pointer' : 'not-allowed',
  fontFamily: 'IBM Plex Sans, sans-serif', opacity: active ? 1 : 0.5,
});

const ghostBtnSt: React.CSSProperties = {
  flex: 1, background: 'rgba(123,110,246,0.1)', color: '#9d90f8', fontSize: 12, fontWeight: 600,
  padding: '9px 14px', borderRadius: 9, border: '0.5px solid rgba(123,110,246,0.35)',
